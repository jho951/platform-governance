...
3계층 서비스는 1계층 OSS를 직접 조립하지 않고, 2계층 `platform-governance`가 제공하는 실행 골격과 override 지점만 소비한다.
...
- 3계층 서비스가 1계층 OSS adapter, builder, engine을 직접 조립하는 방식
...
- `audit`, `config`, `engine`은 adapter와 조립 로직을 분리해 가진다. 현재 engine은 platform `GovernancePolicyPlugin` chain이고, 1계층 `plugin-policy-engine` API adapter는 아니다.
