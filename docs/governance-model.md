...
내부적으로 governance audit 기록은 platform-owned recorder chain을 통해 audit library로 전달한다.
외부 소비자가 production 감사 출력 대상을 추가할 때의 공식 SPI는 `GovernanceAuditSink`다.
...
외부 `GovernanceAuditRecorder` bean override는 mainline starter에서 무시한다. 서비스는 `GovernanceAuditSink`와 `GovernanceAuditRecorder`를 사용해 출력 대상을 확장한다.
서비스는 raw `AuditSink`, `AuditLogger`, `AuditEvent` 대신 `GovernanceAuditSink`, `GovernanceAuditRecorder`, `IdentityAuditRecorder`를 사용한다.
...
`auth-service`는 서비스 로컬 이벤트명 대신 위 표준 action을 사용하고, 차이는 `channel`, `provider`, `loginId`, 추가 attributes로 표현한다.
