...
외부 `AuditLogRecorder` fan-out은 기본 비활성이며, `platform.governance.compat.audit-log-recorder-fanout-enabled=true`일 때만 2.x 호환 경로로 허용한다. 이 경로는 2.0.1부터 deprecated, 3.0.0 제거 대상이다.
