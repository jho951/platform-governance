...
외부 `AuditLogRecorder` bean은 mainline starter에서 무시한다. 서비스는 `AuditSink`를 사용해 출력 대상을 확장한다.
서비스가 `AuditSink`, `AuditLogger`, `AuditEvent`를 코드에서 직접 사용하면 `platform-governance-starter` 외에 `io.github.jho951:audit-log-api`를 명시적으로 추가해야 한다.
