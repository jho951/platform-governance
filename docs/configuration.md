...
- audit는 기본 활성화
- audit logger는 내부적으로 `audit-log-core`의 기본 구현을 사용하고, 등록된 `GovernanceAuditSink`가 있으면 platform-owned sink adapter를 통해 composite delivery를 사용한다.
- 등록된 `GovernanceAuditSink`가 없으면 `platform-governance-adapter-auditlog`의 `LoggingAuditSink` fallback으로 governance audit을 애플리케이션 로그에 남긴다.
- identity audit는 `IdentityAuditRecorder` 공개 API를 제공하고 내부에서 audit library adapter로 매핑한다.
...
- 서비스 설정은 `feature-flags.*` prefix만 사용한다.
...
- 외부 `GovernanceAuditRecorder` bean override는 기본적으로 무시한다. 서비스 확장은 `GovernanceAuditSink`와 `GovernanceAuditRecorder`를 사용한다.
...
- 운영 profile에서 명시적 `GovernanceAuditSink` bean이 하나도 없으면 기본적으로 실패한다.
...
운영 감사 출력 대상의 공식 production SPI는 `GovernanceAuditSink` bean이다.
...
`GovernanceAuditRecorder`는 governance audit entry를 publish하는 공식 recorder이며, 외부 override는 mainline starter에서 무시한다. 서비스 확장점은 `GovernanceAuditSink`와 `GovernanceAuditRecorder`다.
