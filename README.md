...
`platform-governance`는 `audit-log`, `policy-config`, platform `GovernancePolicyPlugin` chain, plugin-policy-engine config 기준을 조립해 감사, 정책 조회, 정책 평가, 정책 변경 기록, 위반 대응을 표준화하는 2계층 운영 통제 계층이다.
...
```gradle
dependencies {
    implementation platform("io.github.jho951.platform:platform-governance-bom:2.0.0")
    implementation "io.github.jho951.platform:platform-governance-starter"
}
```

서비스별 차이는 artifact가 아니라 `platform.governance.service-role-preset`과 bean override로 표현한다.
