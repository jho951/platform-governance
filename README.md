...
서비스는 `AuditSink`, `PolicyConfigSource`, `GovernanceDecisionEngine` 같은 공식 SPI만 보아야 하며, `AuditLogRecorder` 같은 adapter 성격의 내부 구현 타입을 extension point처럼 직접 다루지 않는 것을 원칙으로 한다.
